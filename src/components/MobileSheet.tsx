...
interface Props {
  ...
  setFormattingStandard: (v: FormattingStandard) => void;
  citationStyle: CitationStyle;
  setCitationStyle: (v: CitationStyle) => void;
  conferenceFormat: ConferenceFormat;
  setConferenceFormat: (v: ConferenceFormat) => void;
  onOpenStyles: () => void;
  onOpenPreview: () => void;
}
      ...
      <div className={`sheet-modal${closing ? " closing" : ""}`}>
        <div className="px-5 pt-4 pb-2 shrink-0">
          ...
          <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>
            Select the formatting rule and run options.
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4 space-y-4">
          <div>
            ...
            >
              <i className="fa-solid fa-compass mr-1" /> Formatting Rule
            </p>
            <div className="flex gap-2">
              {FORMATTING_STANDARDS.map((standard) => {
                const active = formattingStandard === standard.value;
                return (
                  <button
                    key={standard.value}
                    type="button"
                    onClick={() => setFormattingStandard(standard.value)}
                  ...
                  >
                    ...
                    >
                      <i className={`fa-solid ${standard.icon}`} />
                    </span>
                    <span
                    ...
                    <span
                      className="text-[10px]"
                      style={{
                        color: active ? "var(--text-secondary)" : "var(--text-soft)",
                      }}
                    >
                      {standard.sub}
                    </span>
          ...
          {formattingStandard === "ccc" ? (
            ...
            <div>
              <p
                ...
                style={{ color: "var(--text-muted)" }}
              >
                <i className="fa-solid fa-file-signature mr-1" /> Conference Format
              </p>
              <div className="space-y-2">
                {CONFERENCE_FORMATS.map((fmt) => {
                  const active = conferenceFormat === fmt.value;
                  return (
                    <button
                      key={fmt.value}
                      type="button"
                      onClick={() => setConferenceFormat(fmt.value)}
                      className="w-full flex items-center gap-3 rounded-2xl border-2 px-3 py-3 transition-all text-left"
                      style={
                        active
                          ? {
                            borderColor: "var(--accent)",
          ...
          {formattingStandard === "ccc" && (
            <>
              ...
              <div>
                <button
                  type="button"
                  className="mb-1 w-full flex items-center justify-between rounded-xl px-1 py-1.5 transition cursor-pointer"
